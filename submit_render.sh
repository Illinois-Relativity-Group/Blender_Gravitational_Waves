...
echo "FRAMES='$FRAMES' -> $N frames | array 0-$((A-1))%$CAP | WITH_DISK=$WD DISK_FLAT=$DISK_FLAT HOLE_RADIUS=$HOLE_RADIUS ZSCALE=$ZSCALE SAMPLES=$SAMPLES"
...
jid=$(sbatch --parsable \
  ...
  --export=ALL,FRAMES_FILE="$LIST",ARRAY_SIZE="$A",RENDER_DIR="$RENDER_DIR",OBJ_DIR="$OBJ_DIR",WITH_DENSITY="$WD",DISK_MANIFEST="$DISK_MANIFEST",HOLE_RADIUS="$HOLE_RADIUS",ZSCALE="$ZSCALE",DISK_MARGIN="$DISK_MARGIN",DISK_FLAT="$DISK_FLAT",SAMPLES="$SAMPLES",STRIDE="$STRIDE" \
